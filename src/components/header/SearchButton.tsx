...
function SearchPanel() {
  ...
  if (import.meta.env.DEV) {
    resultList = (
      <div className="h-full flex items-center justify-center">
        <div className="flex gap-2">
          ...
          <div>
            <div className="font-semibold mb-1">Sorry...</div>
            <div className="text-sm">この機能は Pagefind に基づいています。ビルド後にもう一度おためしください。</div>
          </div>
        </div>
      </div>
    )
    ...
    resultList = (
      <>
        <div className="text-sm px-3 mb-2">{results.length} 件</div>
        {results.map((item) => {
          return (
            <a
              href={item.url}
